...
def build_model(config):
    ...
    return model.to(device)  # CPUにモデルを移動
...
def train_one_epoch(dataloader, model, optimizer, epoch):
    ...
    for step, data in enumerate(dataloader):
        # forward
        data = data.to(device)  # CPUにデータを移動
        ret = model(data)
        loss = ret["loss"]
        # backward
        optimizer.zero_grad()
...
def eval_once(dataloader, model):
    ...
    for data, targets in dataloader:
        data, targets = data.to(device), targets.to(device)  # CPUに移動
        with torch.no_grad():
            ret = model(data)
...
def evaluate(args):
    config = yaml.safe_load(open(args.config, "r"))
    model = build_model(config)
    checkpoint = torch.load(args.checkpoint, map_location=device)  # CPUでロード
